"""Spec-sheet scene layers for 데모 찍먹 (Korean) — the teardown card.

    python scripts/motion/render_spec_layers_ko.py '[["엔진","유니티 / 모노"],["팀","한국, 샌디 플로어"],
        ["자체 모듈","여덟 개"],["게임 코드","Assembly-CSharp 5 KB"],["함정","스테이지 세 개"]]'         --verdict "찍먹 추천" [--width 1920] [--height 1080] [--section 14.2]

A copy of render_spec_layers.py with **only the fonts changed**: Anton, Inter and
JetBrains Mono carry no Hangul, so Korean text renders as tofu in them. Everything
else -- the seven layer names (sc_header/, sc_row_0..4/, sc_stamp/), the timings, the
motion -- is identical, so build_plan.py and its SFX cue table need no change.

Display and label text use Noto Sans KR (Bold/Medium). Values stay monospaced where
they are ASCII, but fall back to Noto Sans KR the moment a value contains Hangul --
a mixed line like "Assembly-CSharp 5 KB" must not lose half its glyphs.
"""
...
ANTON = 'C:/Windows/Fonts/NotoSansKR-VF.ttf'   # display: Hangul-capable
INTER = 'C:/Windows/Fonts/NotoSansKR-VF.ttf'   # labels
# The renderer writes its PNG sequences into the CWD, so it is run from the episode's
# layer folder -- asset paths must therefore resolve against the repo, not the CWD.
_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
MONO = os.path.join(_ROOT, 'assets', 'fonts', 'JetBrainsMono-Bold.ttf')
HEADER = opt.get('header', '이렇게 만들었습니다')
VERDICT = opt.get('verdict', '찍먹 추천')
VERDICT_LABEL = opt.get('verdict-label', '판정')
_pos = [a for a in sys.argv[1:] if not a.startswith('--')]
# Defaults document the intent: five dev-facing facts, not five opinions.
specs = json.loads(_pos[0]) if _pos else [['엔진', 'Unity / Mono'], ['팀', '한국 인디'],
                                          ['자체 모듈', '여덟 개'], ['게임 코드', '5 KB'],
                                          ['함정', '스테이지 세 개']]
...
def _face(text, size, weight='Bold'):
    """JetBrains Mono for pure-ASCII values, Noto Sans KR the moment Hangul appears.
    A mixed value like "Assembly-CSharp 5 KB" is fine in mono; "스테이지 세 개" is not --
    the mono face has no Hangul and would draw tofu."""
...
def mono_fitted(text, box_w, start=52, floor=26):
    ...
    size = start
    while size > floor and _face(text, size).getlength(text) > box_w:
        size -= 2
    return _face(text, size)
...
def stamp_img():
    ...
    # Korean verdicts are short ("찍먹 추천") and read better on one line.
    size = 92
    # Hangul glyphs are square and wider per character than Latin, and the stamp is
    # rotated -10 degrees, so the corners need more room than the English version.
    while size > 40 and max(inter(size, 'Bold').getlength(w) for w in words) > S - 170:
        size -= 4
    fv = inter(size, 'Bold')
    y = c - 46
    for w in words:
